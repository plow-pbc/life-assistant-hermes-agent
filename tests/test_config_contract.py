...
def test_the_hermes_volumes_are_exactly_these():
    ...
    assert set(override()["services"]["hermes"]["volumes"]) == {
        ...
        "${AGENT_DIR:?set by agent-mgr from the registry}"
        "/docker/s6-rc.d/agent-index:/etc/s6-overlay/s6-rc.d/agent-index:ro",
        "${AGENT_DIR:?set by agent-mgr from the registry}"
        "/docker/s6-rc.d/user/contents.d/agent-index"
        ":/etc/s6-overlay/s6-rc.d/user/contents.d/agent-index:ro",
    }
