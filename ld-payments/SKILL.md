...
3. **Proceed only after the owner approves through that flow.** The payment is
   carried out by the vaulted Latch browser tools (`mcp__latch__plow_browser_*`
   with `mcp__latch__plow_vault`) — the vault fills the banking credentials into the site; they
