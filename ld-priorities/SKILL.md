...
One list, one file: `/opt/data/ld/priorities.json` — its name, its open items
**in rank order**, the ranking rules you have learned, and what got done. You
never edit that file by hand. Every change goes through one CLI, by absolute
path (a chat turn's working directory is not the skill directory):

    /opt/data/skills/ld-priorities/scripts/priorities.py <command>
...
5. `post`. It composes the tile into the fixed handoff file —
   `/opt/data/ld/priorities-text` — and ships it through the shared kiosk
   helper as card 6, type `priorities`, http(s)-allowed, no redirects, and
   fails loudly on any non-200 response. If it prints `NO WALL`, the wall is
   not set up yet: the list still works, say nothing about the wall.
...
unanswered email is `ld-morning-triage`'s alert, not a to-do). Not per-person
— it is the household's list, and a trusted group member's "add X" is as good
as the owner's.
