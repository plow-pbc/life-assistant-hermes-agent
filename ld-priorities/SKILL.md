---
name: ld-priorities
description: The household to-do list — every todo, task, or "add this to the list" the owner or a trusted household member gives you in chat ("create a todo: check who can come Oct 12", "add X to our to-do list", "remind me to renew the passports", "X is done", "what's next?", "call the list Weekend jobs", "no, the passport is more urgent"). Items live in this list and show on the wall card; the assistant owns the order and keeps the rules it learns. Not Apple Reminders: use that only when the owner names the Reminders app or their iPhone. Never on a schedule.
---
