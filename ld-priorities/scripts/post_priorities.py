...
and the root test_wrappers.py can read them, and TITLE from the manifest's list
name at run time. `priorities.py post` composes the tile into MESSAGE_FILE and
posts in-process (it needs the same constants live for its own --dry-run tests,
which rebind priorities.MESSAGE_FILE rather than this file's literal); this
wrapper is what a standalone retry after a failed send invokes instead.
"""
import os
import sys
...
post_to_kiosk.MESSAGE_FILE = "/opt/data/ld/priorities-text"
